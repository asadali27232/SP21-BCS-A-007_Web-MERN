...
function removeFromCart(productToBeRemovedFromCart) {
    ...
    cartBoxPrd.removeChild(
        cartBoxPrd.querySelector('#' + productToBeRemovedFromCart)
    );
    billing();
}
...
function billing() {
    ...
    for (let i = 0; i < cartBox.children.length; i++) {
        gst[i] = parseFloat(
            parseFloat(cartBox.children[i].children[1].innerText.slice(3)) *
                parseFloat(
                    cartBox.children[i].children[2].children[1].innerText
                ) *
                0.17
        ).toFixed(2);
        ...
        amount[i] = parseFloat(
            parseFloat(gst[i]) +
                parseFloat(
                    ...
                    parseFloat(
                        cartBox.children[i].children[1].innerText.slice(3)
                    )
        ).toFixed(2);

        billBox.children[1].innerHTML += `<div class="table-row">
                <p class="smaller left">${
                    cartBox.children[i].children[0].innerText
                }</p>
                <p class="smaller">${
                    cartBox.children[i].children[2].children[1].innerText
                }</p>
                <p class="smaller">${cartBox.children[
                    i
                ].children[1].innerText.slice(3)}</p>
    ...
    document.getElementById('total-amount').innerText = (
        totalAmount -
        totalGst -
        (totalAmount - totalGst) * (parseFloat(discount) / 100.0) +
        totalGst
    ).toFixed(2);
    ...
    document.getElementById('cust-balance').innerText = (
        parseInt(received) -
        (totalAmount -
            totalGst -
            (totalAmount - totalGst) * (parseFloat(discount) / 100.0) +
...
function printInvoice() {
    ...
    WinPrint.document.getElementsByTagName('head')[0].innerHTML = `<style>
            * {
            padding: 0;
            margin: 0;
            }
